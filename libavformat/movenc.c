...
static int mov_write_dca3_tag(AVFormatContext *s, AVIOContext *pb, MOVTrack *track){
    ...
    sr_idx         = AV_RB8(track->par->extradata  + 1);  /* samping rate index */
    ...
    put_bits(&pb_dca3, 4, sr_idx);   /* samping rate index */
    ...
    if (version == 2) {
        ...
        if (track->mode == MODE_MOV) {
            ...
            if (track->par->codec_id == AV_CODEC_ID_FLAC ||
                ...
                track->par->codec_id == AV_CODEC_ID_AVS3_AUDIO) {
                avio_wb16(pb, track->par->channels);
            } else {
                avio_wb16(pb, 2);
            }
...
static const AVCodecTag codec_mp4_tags[] = {
    ...
    { AV_CODEC_ID_AVS3_AUDIO,      MKTAG('a', 'v', '3', 'a') },
    { AV_CODEC_ID_NONE,               0 },
};
